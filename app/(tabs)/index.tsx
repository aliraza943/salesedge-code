...
export default function HomeScreen() {
  ...
  const handleSaveGoal = async () => {
    ...
    try {
      ...
      Alert.alert("Error", "Failed to save sales goal. Please try again.");
    } finally {
        ...
        <View className="flex-1 justify-end" style={{ backgroundColor: "rgba(0,0,0,0.5)" }}>
          <View className="rounded-t-3xl p-5 pb-10" style={{ backgroundColor: colors.background }}>
            <View className="flex-row items-center justify-between mb-5">
              <TouchableOpacity onPress={() => setShowEditGoal(false)} activeOpacity={0.7} disabled={isSavingGoal}>
                <Text className="text-base" style={{ color: isSavingGoal ? colors.muted + "80" : colors.muted }}>Cancel</Text>
              </TouchableOpacity>
              <Text className="text-lg font-bold" style={{ color: colors.foreground }}>Edit Sales Goal</Text>
              <TouchableOpacity onPress={handleSaveGoal} activeOpacity={0.7} disabled={isSavingGoal} style={{ minWidth: 44, alignItems: "flex-end" }}>
                {isSavingGoal ? (
                  <ActivityIndicator size="small" color={colors.primary} />
                ) : (
